...
The sequencer language does not (as far as I can tell) have a `while` loop or similar structure.
This means that scripting directly in this language is tedious, especially if you have to do
something like flash in intervals all night long.  My approach is to implement 3 Python scripts
(to start with) which take simple inputs and make a sequencer script that can then be loaded. 
